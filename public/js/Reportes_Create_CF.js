    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        function handleGroupImages() {
            ...
            // Separar imágenes seleccionadas (con checkbox marcado)
            const selectedImages = allContainers.filter(div => {
                const checkbox = div.querySelector('[type="checkbox"]');
                return checkbox && checkbox.checked;
            });

            // Separar imágenes NO seleccionadas
            const remainingImages = allContainers.filter(div => {
                const checkbox = div.querySelector('[type="checkbox"]');
                return !checkbox || !checkbox.checked;
            });
            ...
            // 1️⃣ Insertar primero todas las imágenes marcadas (una por hoja)
            selectedImages.forEach(div => {
                const singleDiv = document.createElement('div');
                singleDiv.classList.add('col-12', 'border', 'p-3', 'rounded', 'mb-3', 'bg-light');
                singleDiv.innerHTML = `
                    <div class="row">
                        <div class="col-sm-6 mx-auto">${div.innerHTML}</div>
                    </div>
                `;
                container.appendChild(singleDiv);
            });

            // 2️⃣ Agrupar las restantes de 2 en 2 con un solo comentario
            for (let i = 0; i < remainingImages.length; i += 2) {
                const groupDiv = document.createElement('div');
                groupDiv.classList.add('col-12', 'border', 'p-3', 'rounded', 'mb-3');
                groupDiv.innerHTML = `
                    <div class="row">
                        <div class="col-sm-6">${remainingImages[i].innerHTML}</div>
                        ${remainingImages[i + 1] ? `<div class="col-sm-6">${remainingImages[i + 1].innerHTML}</div>` : ''}
                    </div>
                    <textarea class="form-control mt-2" name="comments_grouped[]" placeholder="Comentario para estas imágenes"></textarea>
                `;
