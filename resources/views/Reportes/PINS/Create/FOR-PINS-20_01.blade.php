...
<section class="content w-100">
    <div class="card w-100 p-3">
        <div class="card-body w-100">
            <form id="FOR-PINS-20-01" action="{{route('Reportes_FOR_PINS_20_01.store')}}" method="post" enctype="multipart/form-data">
                ...
                <div style="margin-bottom: 2px;"></div>
                    ...
                    <div class="col-sm-4">
                        <div class="form-group">
                            <label class="col-form-label" for="inputSuccess">Tipo de  Recubrimiento o Aislamiento</label>
                            <input type="text" class="form-control  inputForm @error('tip_ais') is-invalid @enderror" name="Detalles_Generales[tip_ais]"  placeholder="Ejemplo:" value="{{old('Detalles_Generales.tip_ais')}}">
                            @error('tip_ais')
                                    <div class="invalid-feedback"><span>{{ $message }}</span></div>
                            @enderror
                        </div>
                    ...
                    <div class="col-sm-4">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control  inputForm @error('ins') is-invalid @enderror" name="Detalles_Generales[ins]"  placeholder="Ejemplo:  " value="{{old('Detalles_Generales.ins')}}">
                            @error('ins')
                                    <div class="invalid-feedback"><span>{{ $message }}</span></div>
                            @enderror
                        </div>
                    ...
                    <div class="col-sm-4">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control  inputForm @error('Nom_pz') is-invalid @enderror" name="Detalles_Generales[Nom_pz]"  placeholder="Ejemplo:  " value="{{old('Detalles_Generales.Nom_pz')}}">
                            @error('Nom_pz')
                                    <div class="invalid-feedback"><span>{{ $message }}</span></div>
                            @enderror
                        </div>
...
<script src="{{ asset('js/Reportes_Create-For-01-20.js') }}"></script> <!--Se realiza el cambio a las imagenes que se tienene de foto y comentario-->
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        const form = document.getElementById('FOR-PINS-20-01');
        if (!form) return;

        // Guardar en localStorage al escribir
        //form.querySelectorAll('input:not([type="file"]), textarea, select').forEach(function (el) {
          //  el.addEventListener('input', function () {
            //    localStorage.setItem('FOR-PINS-20-01_' + el.name, el.value);
        ...
        form.querySelectorAll('input:not([type="file"]), textarea, select').forEach(function (el) {
            if (!el.value) {
                const value = localStorage.getItem('FOR-PINS-20-01_' + el.name);
        ...
        form.addEventListener('submit', function () {
            form.querySelectorAll('input:not([type="file"]), textarea, select').forEach(function (el) {
                localStorage.removeItem('FOR-PINS-20-01_' + el.name);
                //localStorage.clear();
            });
        });
