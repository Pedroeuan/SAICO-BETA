                ...
                <section class="content w-100">
                    <div class="card w-100 p-3">
                        <div class="card-body w-100">
                            <form id="FOR-PINS-05_02" action="{{route('Reportes_FOR_PINS_05_02.store')}}" method="post" enctype="multipart/form-data">
                                @csrf
                                <div class="row">
                                ...
                                <div style="margin-bottom: 2px;"></div>
                                    ...
                                    <div class="col-sm-4">
                                        <div class="form-group">
                                    <label class="col-form-label">¿Cliente existente?<span class="ml-3">
                                    <label class="mr-2">
                                    ...
                                    <select id="campoClienteSelect" class="form-select" name="ClienteSelect">
                                        ...
                                        @foreach($Clientes as $Cliente)
                                            <option value="{{ $Cliente->Cliente }}"> {{ $Cliente->Cliente }}
                                            </option>
                                        @endforeach
                                    </select>
                                    ...
                                    <div class="col-sm-3">
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control  inputForm" id="freccInputA" name="Datos_Equipo[FREC_TRANSDUCTOR]" placeholder="" value="{{old('Datos_Equipo.N_S_EQUIPO')}}">
                                        </div>
                                    </div>
...
<script>
    ...
    $(document).ready(function() {
        let tituloCount = 0; // Contador para títulos
        let rowCount = 0; // Contador para filas dentro de cada título
        let rowCountGlobal = 0; // Contador global para todas las filas, sin importar el título 

        function restoreData() {
            const data = JSON.parse(sessionStorage.getItem('dynamicTableData') || 'null');
                if(!data) return;
                //Helper para restaurar los datos en la tabla
                const fieldNames = [
                    'no_junta',
                    'no_indicacion',
                    'clasificacion',
                ...
                const placeholders = { // configurar los placeholders para cada campo
        ...
        $('#addTituloBtn').click(function () {
            ...
            const titleId = `titulo_${tituloCount}_${Date.now()}`; // ID único basado en el contador y timestamp
            let newTitle = `
            <tr class="titulo-row" data-titulo="${titleId}">
                <td colspan="12">
                    <div class="d-flex justify-content-between align-items-center">
            ...
            if (formId && typeof saveData === 'function') {
                saveData(formId);
            }
        });
    ...
    document.addEventListener('DOMContentLoaded', function () {
        const form = document.getElementById('FOR-PINS-05_02');
        if (!form) return;

        // Guardar en localStorage al escribir
        //form.querySelectorAll('input:not([type="file"]), textarea, select').forEach(function (el) {
          //  el.addEventListener('input', function () {
            //    localStorage.setItem('FOR-PINS-05_02_' + el.name, el.value);
            //});
        //});
        ...
        form.querySelectorAll('input:not([type="file"]), textarea, select').forEach(function (el) {
            if (!el.value) {
                const value = localStorage.getItem('FOR-PINS-05_02_' + el.name);
        ...
        form.addEventListener('submit', function () {
            form.querySelectorAll('input:not([type="file"]), textarea, select').forEach(function (el) {
                localStorage.removeItem('FOR-PINS-05_02_' + el.name);
            });
        });
    });
