    ...
    <section class="content w-100">
        <div class="card w-100 p-3">
            <div class="card-body w-100">
                <form id="FOR-02-PRO-INS-10" action="{{route('Reportes_FOR_PINS_11_01.store')}}" method="post" enctype="multipart/form-data">
                    @csrf
...
$(document).ready(function() {
        ...
        (data.longs || []).forEach(function(l){
            ...
            if ($rowsBlock.length >= 10) { // si hay al menos 11 filas en el bloque
                const $nfila = $rowsBlock
                    .not('.long-row')
                    .eq(9); // fila índice 10 = fila 11 (0-based)
    ...
    document.addEventListener('DOMContentLoaded', function () {
        const form = document.getElementById('FOR-PINS-11_01');
        if (!form) return;

        // Guardar en localStorage al escribir
        //form.querySelectorAll('input:not([type="file"]), textarea, select').forEach(function (el) {
          //  el.addEventListener('input', function () {
            //    localStorage.setItem('FOR-PINS-11_01_' + el.name, el.value);
            //});
        //});
        ...
        form.querySelectorAll('input:not([type="file"]), textarea, select').forEach(function (el) {
            if (!el.value) {
                const value = localStorage.getItem('FOR-PINS-11_01_' + el.name);
        ...
        form.addEventListener('submit', function () {
            form.querySelectorAll('input:not([type="file"]), textarea, select').forEach(function (el) {
                localStorage.removeItem('FOR-PINS-11_01_' + el.name);
                //localStorage.clear();
            });
        });
    });
