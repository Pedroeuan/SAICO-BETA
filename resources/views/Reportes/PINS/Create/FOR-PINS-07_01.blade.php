                ...
                <section class="content w-100">
                    <div class="card w-100 p-3">
                        <div class="card-body w-100">
                            <form id="FOR-PINS-07_01" action="{{route('Reportes_FOR_PINS_07_01.store')}}" method="post" enctype="multipart/form-data">
                                @csrf
                                <div class="row">
                                ...
                                <!-- check btton -->
                                    ...
                                    <div class="col-sm-50 d-flex justify-content-center">
                                        <div class="form-group text-center">
                                            ...
                                            <input type="hidden" name="Datos_Equipo[ID_TRANSDUCTOR]" id="IDInputA" value="{{ old('Datos_Equipo.ID_TRANSDUCTOR') }}">
                                        </div>
                                    </div>
...
$(document).ready(function() {
    ...
    document.addEventListener('DOMContentLoaded', function () {
        const form = document.getElementById('FOR-PINS-07_01');
        if (!form) return;

        form.querySelectorAll('input:not([type="file"]), textarea, select').forEach(function (el) {
            el.addEventListener('input', function () {
                if (el.closest('#dynamicTable')) return;
                localStorage.setItem('FOR-PINS-07_01_' + el.name, el.value);
            });
        });

        form.querySelectorAll('input:not([type="file"]), textarea, select').forEach(function (el) {
            if (!el.value) {
                const value = localStorage.getItem('FOR-PINS-07_01_' + el.name);
        ...
        form.addEventListener('submit', function () {
            form.querySelectorAll('input:not([type="file"]), textarea, select').forEach(function (el) {
                localStorage.removeItem('FOR-PINS-07_01_' + el.name);
            });
        });
    });
</script>
