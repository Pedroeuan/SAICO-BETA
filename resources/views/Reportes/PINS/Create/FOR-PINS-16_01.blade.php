                ...
                <section class="content w-100">
                    <div class="card w-100 p-3">
                        <div class="card-body row w-100">
                            <form id="FOR-PINS-16_01" action="{{route('Reportes_FOR_PINS_16_01.store')}}" method="post" enctype="multipart/form-data">
                                @csrf 
                                <div class="row">
                    ...
                    <p> 
                                    <!--***************************************** INICIO RESULTADOS INFORME DE INSPECCIÓN VISUAL A ELEMENTOS DE TUBERÍAS DE PROCESO *****************************************-->
...
/*Juntas-Resultados CAMBIAR JUNTAS CHECAR*/
    ...
    document.addEventListener('DOMContentLoaded', function () {
        const form = document.getElementById('FOR-PINS-16_01');
        if (!form) return;

        // Guardar en localStorage al escribir
        //form.querySelectorAll('input:not([type="file"]), textarea, select').forEach(function (el) {
          //  el.addEventListener('input', function () {
            //    localStorage.setItem('FOR-PINS-16_01_' + el.name, el.value);
            //});
        //});

        form.querySelectorAll('input:not([type="file"]), textarea, select').forEach(function (el) {
            el.addEventListener('input', function () {
                if (el.closest('#dynamicTable') || el.closest('#componentesTable')) return; // Ignora inputs de las tablas
                localStorage.setItem('FOR-PINS-16_01_' + el.name, el.value);
            });
        });
        ...
        form.querySelectorAll('input:not([type="file"]), textarea, select').forEach(function (el) {
            if (!el.value) {
                const value = localStorage.getItem('FOR-PINS-16_01_' + el.name);
        ...
        form.addEventListener('submit', function () {
            form.querySelectorAll('input:not([type="file"]), textarea, select').forEach(function (el) {
                localStorage.removeItem('FOR-PINS-16_01_' + el.name);
                //localStorage.clear();
            });
        });
    });
