...
<section class="content w-100">
    <div class="card w-100 p-3">
        <div class="card-body w-100">
            <form id="FOR-PINS-21_01" action="{{route('Reportes_FOR_PINS_21_01.store')}}" method="post" enctype="multipart/form-data">
                ...
                <div style="margin-bottom: 2px;"></div>
                    ...
                    <div class="col-sm-12">
                        <div class="form-group">
                            ...
                            <textarea class="form-control is-waning @error('Datos_Equipo.Observaciones') is-invalid @enderror" id="inputSuccess" name="Datos_Equipo[Observaciones]" placeholder="Ejemplo: LA INSPECCIÓN SE REALIZÓ DE LADO A Y B">{{ old('Datos_Equipo.Observaciones') }}</textarea>
...
$(document).ready(function() {
    ...
    document.addEventListener('DOMContentLoaded', function () {
        const form = document.getElementById('FOR-PINS-21_01');
        if (!form) return;

        // Guardar en localStorage al escribir
        //form.querySelectorAll('input:not([type="file"]), textarea, select').forEach(function (el) {
          //  el.addEventListener('input', function () {
            //    localStorage.setItem('FOR-01-PRO-INS-21_' + el.name, el.value);
        ...
        form.querySelectorAll('input:not([type="file"]), textarea, select').forEach(function (el) {
            el.addEventListener('input', function () {
                ...
                localStorage.setItem('FOR-PINS-21_01_' + el.name, el.value);
            });
        });
        ...
        form.querySelectorAll('input:not([type="file"]), textarea, select').forEach(function (el) {
            if (!el.value) {
                const value = localStorage.getItem('FOR-PINS-21_01_' + el.name);
        ...
        form.addEventListener('submit', function () {
            form.querySelectorAll('input:not([type="file"]), textarea, select').forEach(function (el) {
                localStorage.removeItem('FOR-PINS-21_01_' + el.name);
                //localStorage.clear();
            });
        });
    });
