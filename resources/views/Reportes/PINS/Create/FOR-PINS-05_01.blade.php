                ...
                <section class="content w-100">
                    <div class="card w-100 p-3">
                        <div class="card-body w-100">
                            <form id="FOR-PINS-05_01" action="{{route('Reportes_FOR_PINS_05_01.store')}}" method="post" enctype="multipart/form-data">
                                @csrf
                                <div class="row">
...
<script>
    ...
    $(document).ready(function() {
        ...
        const FORM_ID = document.querySelectorAll("form")[1]?.id || 'FOR-PINS-05_01';
        const CONFIG = {
    ...
    document.addEventListener('DOMContentLoaded', function () {
        const form = document.getElementById('FOR-PINS-05_01');
        if (!form) return;
        ...
        form.querySelectorAll('input:not([type="file"]), textarea, select').forEach(function (el) {
            el.addEventListener('input', function () {
                ...
                localStorage.setItem('FOR-PINS-05_01_' + el.name, el.value);
            });
        });
        ...
        form.querySelectorAll('input:not([type="file"]), textarea, select').forEach(function (el) {
            if (!el.value) {
                const value = localStorage.getItem('FOR-PINS-05_01_' + el.name);
        ...
        form.addEventListener('submit', function () {
            form.querySelectorAll('input:not([type="file"]), textarea, select').forEach(function (el) {
                localStorage.removeItem('FOR-PINS-05_01_' + el.name);
                //localStorage.clear();
            });
        });
    });
