...
<section class="content w-100">
    <div class="card w-100 p-3">
        <div class="card-body w-100">
            <form id="FOR-PINS-06_01" action="{{route('Reportes_FOR_PINS_06_01.store')}}" method="post" enctype="multipart/form-data">
                @csrf
                <div class="row">
...
<script>
    ...
    function restoreData() {//-----------------------------------------------------------Reemplazar todo el resotedara
        ...
        $('#addTituloBtn').click(function () {
            ...
            let newTitle = `
            <tr class="titulo-row" data-titulo="titulo_${titleId}">
                <td colspan="10">
                    <div class="d-flex justify-content-between align-items-center">
    ...
    document.addEventListener('DOMContentLoaded', function () {
        const form = document.getElementById('FOR-PINS-06_01');
        if (!form) return;

        form.querySelectorAll('input:not([type="file"]), textarea, select').forEach(function (el) {
            el.addEventListener('input', function () {
                if (el.closest('#dynamicTable')) return; // Ignora inputs de la tabla
                localStorage.setItem('FOR-PINS-06_01_' + el.name, el.value);
            });
        });
        ...
        form.querySelectorAll('input:not([type="file"]), textarea, select').forEach(function (el) {
            if (!el.value) {
                const value = localStorage.getItem('FOR-PINS-06_01_' + el.name);
        ...
        form.addEventListener('submit', function () {
            form.querySelectorAll('input:not([type="file"]), textarea, select').forEach(function (el) {
                localStorage.removeItem('FOR-PINS-06_01_' + el.name);
                //localStorage.clear();
            });
            //localStorage.clear();
        });
