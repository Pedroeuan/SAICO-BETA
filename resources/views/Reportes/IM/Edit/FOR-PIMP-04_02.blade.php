            ...
            <form id="FOR-PIMP-04_02" action="{{route('Reportes_FOR_PIMP_04_02.update', $id)}}" method="post" enctype="multipart/form-data">
                ...
                @if ($errors->any())
                    <div class="alert alert-danger" role="alert">
                        <strong>No se pudo actualizar el reporte:</strong>
                        <ul class="mb-0 mt-1">
                ...
                <div style="margin-bottom: 2px;"></div>
                    ...
                    {{-- Composición química reutilizada por el Edit de 04_02. --}}
                    @php
                        ...
                        $columnasXrfSeleccionadas = old('XRF_Columnas', $normaHistoricaXrf['Columnas_Seleccionadas'] ?? []);
                    @endphp
                    ...
                    <div class="col-12">
                        <div class="form-group border rounded p-3 bg-light">
                            ...
                            <small class="form-text text-muted">
                                Suba un solo PDF y seleccione hasta tres disparos. Al cargar uno nuevo se reemplaza el análisis XRF anterior.
                            </small>
                            <div class="d-flex flex-wrap mt-2">
                                @foreach(range(1, 20) as $columnaXrf)
                                    <label class="form-check mr-4 {{ in_array($columnaXrf, array_map('intval', $columnasXrfSeleccionadas)) ? '' : 'd-none' }}">
                                        <input class="form-check-input columna-xrf" type="checkbox" name="XRF_Columnas[]"
                                            value="{{ $columnaXrf }}" @checked(in_array($columnaXrf, $columnasXrfSeleccionadas))>
                                        {{ $columnaXrf }}
                                    </label>
                                @endforeach
                            </div>
                            ...
                            @error('XRF_Columnas')<div class="invalid-feedback d-block">{{ $message }}</div>@enderror
...
@php($xrfExtractionRoute = \Illuminate\Support\Facades\Route::has('Reportes_FOR_PIMP_04_02.extraer_analisis') ? route('Reportes_FOR_PIMP_04_02.extraer_analisis') : url('/Reportes_FOR_PIMP_04_02/extraer-analisis'))
@php($xrfFormId = 'FOR-PIMP-04_02')
