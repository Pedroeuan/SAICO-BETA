                    ...
                    <thead>
                        <tr>
                            <th rowspan="4" style="width: 450%; font-size: 9pt;">
                                INFORME DE INSPECCIÓN CON PARTÍCULAS MAGNÉTICAS
                            </th>
                            ...
                            <th style="width: 100%;">FOR-PINS-03/02</th>
                            <th rowspan="4" style="width: 90%;">
                            ...
                            <thead>
                                ...
                                @php
                                    $tablaCombinacionConfig = collect($tablaCombinacionConfig ?? [])->map(function ($item) {
                                        return [
                                            'groupId' => $item['groupId'] ?? 'sin_titulo',
                                            'field' => $item['field'] ?? '',
                                            'startRow' => isset($item['startRow']) ? (int) $item['startRow'] : -1,
                                    ...
                                    $obtenerCombinacionTabla = function ($groupId, $field, $rowIndex) use ($tablaCombinacionConfig) {
                                        return $tablaCombinacionConfig->first(function ($item) use ($groupId, $field, $rowIndex) {
                                            return ($item['groupId'] ?? 'sin_titulo') === ($groupId ?: 'sin_titulo')
                                                && ($item['field'] ?? '') === $field
                                                && (int) ($item['startRow'] ?? -1) === $rowIndex
                                                && (int) ($item['rowspan'] ?? 1) > 1;
                                        });
                                    };
                                        ...
                                        @if (($item['tipo'] ?? null) == 'fila')
                                            ...
                                            <tr class="juntas">
                                                @foreach ($columnasResultadoPdf as $campoPdf)
                                                    @php
                                                        $combinacionCelda = $obtenerCombinacionTabla($groupId, $campoPdf, $rowIndex);
                                                    @endphp

                                                    @if ($combinacionCelda)
                                                        <td rowspan="{{ $combinacionCelda['rowspan'] }}">
                                                            {{ $item['data'][$campoPdf] ?? '' }}
                                                        </td>
                                            ...
                                            @php
                                                $contadorFilasPorGrupo[$groupId] = $rowIndex + 1;
                                            @endphp
                                        @endif
