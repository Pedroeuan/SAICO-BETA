...
class FOR_PINS_21_01Controller extends Controller
{
    public function Datos_QR($datosParaCrearQR)
    {
        ...
        */
        ...
        $directorioQR = storage_path(
            "app/public/Reportes/FOR_PINS_21_01/{$Contrato}/{$No_Reporte}/QR_REPORTES"
        );
        ...
        $rutaQrPublica =
            "storage/Reportes/FOR_PINS_21_01/{$Contrato}/{$No_Reporte}/QR_REPORTES/"
            . $nombreQR;
        ...
        */
        ...
        $directorioTemporal = storage_path(
            "app/temp_pdfs/FOR_PINS_21_01/{$Contrato}/{$No_Reporte}"
        );
        ...
        */
        ...
        $directorioFinal = storage_path(
            "app/public/Reportes/FOR_PINS_21_01/{$Contrato}/{$No_Reporte}/PDF_UNIFICADO"
        );
        ...
        $rutaPublicaFinal =
            "storage/Reportes/FOR_PINS_21_01/{$Contrato}/{$No_Reporte}/PDF_UNIFICADO/"
            . $nombrePdfFinal;
    ...
    public function FOR_PINS_21_01_store(Request $request)
    {
        $Estatus = "CREADO";
        ...
        */
        ...
        foreach ($request->images_base64 as $index => $base64Image) {
            ...
            $rutaCarpeta = "public/Reportes/FOR_PINS_21_01/{$Contrato}/{$No_Reporte}/Fotos";
            ...
            $imagenesGuardadas[] = [
                'ruta' => "storage/Reportes/FOR_PINS_21_01/{$Contrato}/{$No_Reporte}/Fotos/{$imageName}",
                'comentario' => $request->comments[$index] ?? null, // Guardar comentario si existe
                'una_hoja' => $request->imagen_hoja[$index] ?? 0, // Guardar si la imagen es de una hoja o no
            ];
        }
        ...
        try {
            ...
            Log::error('FOR_01_PRO_INS_21: el reporte se guardo, pero fallo la creacion auxiliar de OS/OC.', [
                'idReportes' => $idReportes,
                'idSolicitud' => $idSolicitud,
                'idPrueba_Aplica' => $idPrueba_Aplica,
                'contrato' => $Contrato,
        ...
        $redirect = redirect()->route('indexINS2', ['contratoSeleccionado' => $contratoSeleccionado, 'Proyecto' => $Proyecto]);

        if ($osOcError) {
            return $redirect->with('warning', 'El reporte 21 se guardo, pero fallo la creacion auxiliar de OS/OC. Revisa el log para mas detalle.');
        }

        return $redirect;
    }

    public function FOR_PINS_21_01_update(Request $request, $id)
    {
        $Estatus = "ACTUALIZADO";
        // Validar los Detalles_Generales
        $validatedData = $request->validate([
        ...
        if ($request->hasFile('Detalles_Generales.Reporte_Firmado')) {
            ...
            $rutaBase = "public/Reportes/FOR_PINS_21_01/{$Contrato}/{$No_Reporte}/Reporte_Firmado";
    ...
    public function FOR_PINS_21_01($id)
    {
        // Encontrar el Reporte, Fotos_Reportes, Firmas_Reportes, Grupo_Juntas_Detalles_Re para actualizar los datos en la base de datos
        $Reporte = reporte::where('idReportes', $id)->first();
        $Grupo_Juntas_Detalles_Re = Grupo_Juntas_Detalles_Re::where('idReportes', $id)->first();
        ...
        $data = [
            'title' => 'Reporte_FOR-PINS-21-01.PDF',
            'QR_PDF' => $qrPdf,
            'Logo' => $Logo,
            //Detalles_Generales
        ...
        $pdf1 = PDF::loadView('Reportes.ReportesPDF.Reporte_FOR_PINS_21_01_PDF', $data)->setPaper('letter', 'portrait');

        // Generar el PDF adicional en orientación vertical
        $pdf2 = PDF::loadView('Reportes.ReportesFotosPDF.Reporte_FOTOS_FOR_PINS_21_01_PDF', $data)->setPaper('letter', 'portrait');
       ...
       // Crear objetos FPDI independientes para contar páginas
        ...
        for ($i = 1; $i <= $pageCount2; $i++) {
            ...
            $combinedPdf->useTemplate($tplId, 0, 0, 210, 297);
            $combinedPdf->SetFont('Arial', 'B', 8);
            $combinedPdf->SetXY(142, -266.5);
            // Para que el conteo sea consecutivo
            $combinedPdf->Cell(0, 10, ($i + $pageCount1) . " de $totalPageCount", 0, 0, 'C');
        }

        return response($combinedPdf->Output('Reporte_FOR_PINS_21_01.PDF', 'I'), 200)
